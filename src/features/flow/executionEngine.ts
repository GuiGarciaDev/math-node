...
function getInputValues(
  ...
  if (node?.data.inputs) {
    ...
    const sortedEdges = incomingEdges.sort((a, b) => {
      const aIdx = inputDefs.findIndex(
        (d) => d.name === (a.targetHandle ?? ""),
      );
      const bIdx = inputDefs.findIndex(
        (d) => d.name === (b.targetHandle ?? ""),
      );
      return aIdx - bIdx;
    });
  ...
  try {
    switch (nodeType) {
      ...
      case "plot": {
        ...
        return { value: points, type: "array" };
      }
